...
class StateExcelReportController(http.Controller):
	@http.route([
		'/state/excel_report/<model("ng.sale.log.wizard"):wizard>',
	...
	def get_state_excel_report(self, wizard=None, **args):
		...
		# loop for all date between selected date
		count_date = (wizard.end_date - wizard.start_date).days + 1
		for date in (wizard.start_date + td(i) for i in range(count_date)):
			# create worksheet/tab per partner
			sheet = workbook.add_worksheet(dt.strftime(date, '%d-%m-%Y'))
			...
			sheet.merge_range('A1:J1', _('M.G.B.I Sales Log of %s') % dt.strftime(date, '%d/%m/%Y'), title_style)
